...
import { deadlineMode$, draftEntryId$, entries$, mapDisplayMode$, panelVisible$, projectedMarkers$, selectedEntryId$ } from "../bindings";
...
export function MapNotesOverlay() {
  ...
  return <div className={styles.layer}>{markers.map(marker => {
    ...
    return <div key={entry.id} className={`${styles.anchor} ${priorityClass} ${openClass} ${doneClass} ${selectedClass}`} style={{ left: `${marker.screenX * 100}%`, top: `${marker.screenY * 100}%` }}>
      <Button variant="flat" className={`${styles.card} ${kindClass} ${priorityClass}`} onSelect={openEntry}>
        <span className={styles.cardHeader}><span className={styles.cardKind}><KindIcon kind={entry.kind} /></span><strong>{entry.title}</strong></span>
        <span className={styles.meta}><span>{category}</span><span>-</span><span className={styles.statusMeta}><StatusIcon status={entry.status} />{statusLabels[entry.status]}</span>{dueText && <><span>-</span><span className={overdue ? styles.overdue : styles.due}>{dueText}</span></>}</span>
      </Button>
      <span className={styles.tail}></span>
      <Button variant="flat" className={`${styles.marker} ${kindClass} ${priorityClass}`} onSelect={openEntry}><KindIcon kind={entry.kind} className={styles.markerIcon} />{overdue && <span className={styles.badge}>!</span>}{entry.status === EntryStatus.Done && <span className={styles.badge}>x</span>}</Button>
    </div>;
  })}</div>;
}
