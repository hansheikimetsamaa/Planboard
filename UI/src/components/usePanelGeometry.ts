// Persists, clamps, and restores user-controlled Planboard panel geometry.
...
function clampGeometry(value: StoredGeometry, constraints: Constraints): StoredGeometry {
  return {
    ...
    height: limit(
      ...
      window.innerHeight * constraints.maxHeightRatio,
    ),
    x: limit(value.x, 0, 1),
    y: limit(value.y, 0, 1),
  };
...
export function usePanelGeometry(
  ...
  const [geometry, setGeometry] = useState<StoredGeometry>(() =>
    clampGeometry(readGeometry(key), constraints),
  );
  const [panelKey, setPanelKey] = useState(0);
  const initialPosition = useMemo<Number2>(
    () => ({ x: geometry.x, y: geometry.y }),
    [geometry.x, geometry.y, panelKey],
  );
  ...
  const onPanelMouseUp = (event: ReactMouseEvent<HTMLElement>) => {
    ...
    setGeometry((current) => {
      const next = clampGeometry(
        { ...current, x: rect.left / availableX, y: rect.top / availableY },
        constraints,
      );
  ...
  const startResize = (event: ReactMouseEvent<HTMLElement>) => {
    ...
    const resized = (next: MouseEvent, current: StoredGeometry) =>
      clampGeometry(
        {
          ...current,
