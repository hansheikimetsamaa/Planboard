...
style={showDetails ? geometry.panelStyle : { ...geometry.panelStyle, height: "auto" }} onMouseUp={geometry.onPanelMouseUp} className={styles.shell} contentClassName={`${styles.content} ${!showDetails ? styles.compactContent : ""}`} header={<div className={styles.header}><strong>Pin placed</strong><span>Complete the note</span></div>} showCloseHint onClose={discard}>
    <div className={styles.sticky}>
      ...
      <input ref={titleRef} type="text" aria-label="Title" value={title} maxLength={160} className={titleMissing ? styles.missing : ""} placeholder="What's happening here?" onChange={event => { setTitle(event.target.value); if (event.target.value.trim()) setTitleMissing(false); }} onKeyDown={event => event.key === "Enter" && save()} />
