...
function PanelContent() {
    ...
    useEffect(() => { const close = (event: KeyboardEvent) => { if (event.key === "Escape" && pendingDelete !== null)
        setPendingDelete(null); }; window.addEventListener("keydown", close); return () => window.removeEventListener("keydown", close); }, [pendingDelete]);
...
function NewEditor({ deadlineMode, onCancel, onCreate }: {
    deadlineMode: DeadlineMode;
    onCancel: () => void;
    ...
    return <div className={styles.editorShell} onKeyDown={event => {
...
}) { const { kindLabels: k, categoryLabels: c, statusLabels: s, priorityLabels: p } = usePlanboardLocale(); const [title, setTitle] = useState(entry.title), [description, setDescription] = useState(entry.description), [kind, setKind] = useState(entry.kind), [category, setCategory] = useState(entry.category), [categoryName, setCategoryName] = useState(entry.categoryName), [status, setStatus] = useState(entry.status), [priority, setPriority] = useState(entry.priority), [real, setReal] = useState(ticksToDateInput(entry.realDueDateTicks)), [game, setGame] = useState(ticksToDateInput(entry.gameDueDateTicks)), [more, setMore] = useState(false);
