...
export function CompatibilityNotice() {
  ...
  return <div className={styles.notice} role="alert"><div><strong>Planboard compatibility notice</strong>{issues.map(issue => <span key={issue}>{issue}</span>)}</div><Button variant="flat" onSelect={() => trigger(Binding.group, Binding.setPanelVisible, true)}>Open Planboard</Button></div>;
}
