// Provides localized labels with safe English fallbacks for the UI.
...
const localized = (
  ...
  fallbacks.map(
    (fallback, index) => translate(`Planboard.${group}.${index}`, fallback) ?? fallback,
  );

export function usePlanboardLocale() {
  const { translate } = useLocalization();
  const t = (key: string, fallback: string) =>
    translate(`Planboard.UI.${key}`, fallback) ?? fallback;
  return {
    t,
